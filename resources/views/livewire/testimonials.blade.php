        ...
        <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
            ...
            <thead>
                <tr class="fw-bolder text-muted">
                    ...
                    <th class="min-w-150px">اسم العميل </th>
                    <th class="min-w-140px">الوظيفة </th>
                    <th class="min-w-140px">المنتج</th>
            ...
            <tbody>
                @forelse( $testimonials as $testimonial )
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                ...
                                <div class="d-flex justify-content-start flex-column">
                                    <a href="#" class="text-dark fw-bolder text-hover-primary fs-6">{{$testimonial->name}}</a>
                        ...
                        <td>
                            <p  class="text-dark fw-bolder text-hover-primary d-block fs-6">{{$testimonial->job}}</p>
                        </td>
                        <td>
                            <p  class="text-dark fw-bolder text-hover-primary d-block fs-6">{{$testimonial->product->name}}</p>
                        </td>
                        <td>
                            <p  class="text-dark fw-bolder text-hover-primary d-block fs-6">{{$testimonial->description}}</p>
                        </td>
                        <td>
                            <div class="d-flex justify-content-end flex-shrink-0">
                                <button wire:click="showForm({{$testimonial->id}})" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1"  data-bs-toggle="modal" data-bs-target="#edit{{$testimonial->id}}">
                                ...
                                {{--edit modal --}}
                                <!-- Modal -->
                                    @if ($show == $testimonial->id)
                                    <div  class="modal fade show" id="edit{{$testimonial->id}}" tabindex="-1" aria-labelledby="edit{{$testimonial->id}}Label" aria-hidden="true">
                                        <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                            <h1 class="modal-title fs-5" id="edit{{$testimonial->id}}Label">تعديل اراء العملاء</h1>
                                            <button type="button" class="btn-close" wire:click="$toggle('show')"></button>
                                        </div>
                                            <div class="modal-body">
                                                <!--begin::Form-->
                                                <form wire:submit.prevent="updateTestimonial({{$testimonial->id}})" enctype="multipart/form-data">
                                                    <div>
                                                        @if (session()->has('message'))
                                                            <div class="alert alert-success">
                                                                {{ session('message') }}
                                    ...
                                    @endif
                                <form wire:submit.prevent="deleteTestimonial({{$testimonial->id}})">
