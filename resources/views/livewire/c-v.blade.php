        ...
        <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
            ...
            <thead>
                <tr class="fw-bolder text-muted">
                    ...
                    <th class="min-w-150px">الملف التعريفي </th>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                <div class="d-flex justify-content-start flex-column">
                                      <a href="{{route('documents.pdf-document', ['id'=> $CV->id])}}" target="_blank">
                        ...
                        <td>
                            <div class="d-flex justify-content-end flex-shrink-0">
                                <button wire:click="showForm({{$CV->id}})" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1"  data-bs-toggle="modal" data-bs-target="#edit{{$CV->id}}">
                                ...
                                <!-- Modal -->
                                    @if ($show == $CV->id)
                                    <div  class="modal fade show" id="edit{{$CV->id}}" tabindex="-1" aria-labelledby="edit{{$CV->id}}Label" aria-hidden="true">
                                        <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                            <h1 class="modal-title fs-5" id="edit{{$CV->id}}Label">تعديل شركة</h1>
                                            <button type="button" class="btn-close" wire:click="$toggle('show')"></button>
                                        </div>
                                            <div class="modal-body">
                                                <!--begin::Form-->
                                                <form wire:submit.prevent="updateCV({{$CV->id}})" enctype="multipart/form-data">
                                                    <div>
                                                        @if (session()->has('message'))
                                                            <div class="alert alert-success">
                                                                {{ session('message') }}
                                    ...
                                    @endif
                                <form wire:submit.prevent="deleteCV({{$CV->id}})">
