        ...
        <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
            ...
            <thead>
                <tr class="fw-bolder text-muted">
                    ...
                    <th class="min-w-150px">اسم الوسم</th>
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                 <div class="d-flex justify-content-start flex-column">
                                    <a href="#" class="text-dark fw-bolder text-hover-primary fs-6">{{$tag->name}}</a>
                                </div>
                            </div>
                        </td>
                        <td>
                            <div class="d-flex justify-content-end flex-shrink-0">
                                <button wire:click="showForm({{$tag->id}})" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1"  data-bs-toggle="modal" data-bs-target="#edit{{$tag->id}}">
                                ...
                                <!-- Modal -->
                                    @if ($show == $tag->id)
                                    <div  class="modal fade show" id="edit{{$tag->id}}" tabindex="-1" aria-labelledby="edit{{$tag->id}}Label" aria-hidden="true">
                                        <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                            <h1 class="modal-title fs-5" id="edit{{$tag->id}}Label">تعديل الوسم</h1>
                                            <button type="button" class="btn-close" wire:click="$toggle('show')"></button>
                                        </div>
                                            <div class="modal-body">
                                                <!--begin::Form-->
                                                <form wire:submit.prevent="updateTag({{$tag->id}})" enctype="multipart/form-data">
                                                    <div>
                                                        @if (session()->has('message'))
                                                            <div class="alert alert-success">
                                                                {{ session('message') }}
